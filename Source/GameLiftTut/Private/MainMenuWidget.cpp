...
void UMainMenuWidget::NativeConstruct() {
	...
	if (GameInstance != nullptr) {
		UGameLiftTutGameInstance* GameLiftTutGameInstance = Cast<UGameLiftTutGameInstance>(GameInstance);
		if (GameLiftTutGameInstance != nullptr) {
			AccessToken = GameLiftTutGameInstance->AccessToken;
		}
	}
			...
			if (JsonObject->HasField("access_token") && JsonObject->HasField("id_token") && JsonObject->HasField("refresh_token")) {
				...
				if (GameInstance != nullptr) {
					...
					if (GameLiftTutGameInstance != nullptr) {
						...
						GameLiftTutGameInstance->SetCognitoTokens(AccessToken, IdToken, RefreshToken);
...
void UMainMenuWidget::SetAveragePlayerLatency() {
	...
	if (GameInstance != nullptr) {
		...
		if (GameLiftTutGameInstance != nullptr) {
			...
			for (float PlayerLatency : GameLiftTutGameInstance->PlayerLatencies) {
				TotalPlayerLatency += PlayerLatency;
			}

			if (TotalPlayerLatency > 0) {
				AveragePlayerLatency = TotalPlayerLatency / GameLiftTutGameInstance->PlayerLatencies.Num();
...
void UMainMenuWidget::OnMatchmakingButtonClicked() {
	...
	if (GameInstance != nullptr) {
		...
		if (GameLiftTutGameInstance != nullptr) {
			AccessToken = GameLiftTutGameInstance->AccessToken;
			MatchmakingTicketId = GameLiftTutGameInstance->MatchmakingTicketId;
		}
	}
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			if (JsonObject->HasField("ticketId")) {
				...
				if (GameInstance != nullptr) {
					UGameLiftTutGameInstance* GameLiftTutGameInstance = Cast<UGameLiftTutGameInstance>(GameInstance);
					if (GameLiftTutGameInstance != nullptr) {
						GameLiftTutGameInstance->MatchmakingTicketId = MatchmakingTicketId;
...
void UMainMenuWidget::OnStopMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	...
	if (GameInstance != nullptr) {
		UGameLiftTutGameInstance* GameLiftTutGameInstance = Cast<UGameLiftTutGameInstance>(GameInstance);
		if (GameLiftTutGameInstance != nullptr) {
			GameLiftTutGameInstance->MatchmakingTicketId = "";
		}
	}
